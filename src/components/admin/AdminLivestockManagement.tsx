...
import { Stethoscope, Edit, Trash2, Plus, X, Search, Image as ImageIcon } from "lucide-react";
...
export function AdminLivestockManagement() {
  ...
  const handleSubmit = async (e: any) => {
    ...
    if (editingItem) {
      await fetch(`/api/admin/livestock-recommendations`, { method: "PATCH", body: JSON.stringify({ id: editingItem.id, ...payload }) });
    } else {
  ...
  const filtered = livestock.filter(l => l.breedName.toLowerCase().includes(searchQuery.toLowerCase()) || l.location.toLowerCase().includes(searchQuery.toLowerCase()));
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <div>
          <h2 className="text-lg font-bold text-gray-900 flex items-center">
            ...
            Livestock Region Management
          </h2>
          <p className="text-xs text-gray-500 mt-1">Manage livestock breed recommendations based on agro-climatic locations.</p>
        </div>
